...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{./oqum/figures/GenericLogicTreeStructure.eps}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{./oqum/figures/SeismicSourceSystem.eps}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{./oqum/figures/GroundMotionSystem.eps}
...
\subsubsection{Simple fault source definition} \label{simpleFaultSource}
...
\newpage
\section{Input Data definition for Deterministic calculator}
Input data for \Verb+Deterministic+ calculation consists of only a single rupture model. The rupture model is defined in a NRML file whose path is given by the \Verb+SINGLE_RUPTURE_MODEL+ key in the hazard section of the configuration file:
\begin{Verbatim}[frame=single, commandchars=\\\{\}, samepage=true]
[\textcolor{red}{HAZARD}]
...
SINGLE_RUPTURE_MODEL=/PATH/TO/RUPTURE/MODEL/FILE
...
\end{Verbatim}
\subsection{Rupture model definition}
The rupture model file contains a \Verb+simpleFaultRupture+ element:
\begin{Verbatim}[frame=single, commandchars=\\\{\},fontsize=\normalsize, samepage=true]
<\textcolor{red}{simpleFaultRupture}>
	<\textcolor{green}{magnitude}>MOMENT_MAGNITUDE</\textcolor{green}{magnitude}>
	<\textcolor{blue}{tectonicRegion}>TECT_REG_TYPE</\textcolor{green}{tectonicRegion}>
	<\textcolor{magenta}{rake}>RAKE_ANGLE</\textcolor{magenta}{rake}>
	<\textcolor{orange}{simpleFaultGeometry}>
		...
	</\textcolor{orange}{simpleFaultGeometry}>
</\textcolor{red}{simpleFaultRupture}>
\end{Verbatim}
A rupture is therefore associated to a moment magnitude, a tectonic region type and a rake angle. The rupture geometry is described through a \Verb+simpleFaultGeometry+ element (see section \ref{simpleFaultSource}).
