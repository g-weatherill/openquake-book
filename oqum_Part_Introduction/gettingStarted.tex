...
By default OpenQuake saves output data into the OpenQuake-Data Base (that is included in the OpenQuake package). To have output data saved into (XML formatted) files the option \Verb+output_type=xml+ must be specified. The complete command to run OpenQuake is therefore:
