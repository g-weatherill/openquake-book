...
Several methodologies to derive vulnerability functions are currently being evaluated and will be a part of the Modellers Toolkit. In addiction, scripts to convert vulnerability curves stored in Excel or ASCII files into NRML are being developed.

\section{Configuration parameters}
For any of the risk calculations, an exposure and a vulnerability model need to be provided. Hence, in the risk section within the configuration file, each path needs to be inserted next to the respective keys. In addiction, in the Classical PSHA-based and Probabilistic Event-based Risk calculators, if loss maps are pretend to be computed, another key needs to be provided stating the set of probabilities of exceedance (one per loss map). 
