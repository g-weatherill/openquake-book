...
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag7.eps}}
\caption{Fault rupture modeling. The fault surface is modeled by a regular mesh (a). Ruptures are modeled as portions of the fault surface mesh. Rupture dimensions (derived from magnitude using scaling relationship and aspect ratio) determine how many grid points are included in each rupture. For a fault (associated to a maximum magnitude equal to 7), examples of magnitude 5 (b), 6 (c), and 7 (d) ruptures are shown. The red dot represent the hypocenter location.}
\label{faultSurface}
\end{center}
\end{figure}
Rupture dimensions are derived by using a magnitude scaling relationship and the aspect ratio (rupture length/ rupture width).
