...
\end{center}
\end{figure}
In the NRML schema, a logic tree structure is defined through the \Verb+logicTree+ element: 
\begin{Verbatim}[frame=single, commandchars=\\\{\}, samepage=true]
...
\item In all branch sets, branch weights must sum to 1.
\end{itemize}
A schematic representation of a Ground Motion System as defined in a tree structure is given in Figure \ref{gmpeLT}.
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{./oqum_Figures/GroundMotionSystem.eps}
\caption{Schematic representation of a Ground Motion System. The information defining a Ground Motion System is placed in a logic tree structure, where each branching level refers to a specific tectonic region type. The number of GMPEs per branching level (that is per tectonic region type) can vary. Each branch set at level $n$ is linked to all branches in the level $n-1$, so the resulting tree structure is symmetric}
\label{gmpeLT}
\end{center}
\end{figure}
Figure \ref{lt2} shows instead an example of Ground Motion System as defined in a \Verb+logicTreeElement+. Each branching level contains only one \Verb+branchSet+, each referring to a different tectonic region type. Each branch set can contain a variable number of GMPEs.
\begin{figure}[htbp]
\begin{center}
\begin{Verbatim}[frame=single, commandchars=\\\{\},fontsize=\scriptsize, samepage=true]
<\textcolor{red}{logicTree} logicTreeID="ID">       
...
\end{itemize}
Figure \ref{areaSrcGridSpacing} shows the effect of the area source discretization parameter. Given the area boundary (white polygon), ruptures' hypocenters are placed inside the area on a regular grid of nodes (red dots), whose spacing is determined by the \Verb+AREA_SOURCE_DISCRETIZATION+ value. Smaller is the grid spacing better the earthquake ruptures will cover the area polygon, however higher will be the number of ruptures to be considered in the ERF (hence higher will be the computation time).\\
\begin{figure}[!htbp]
\begin{center}
\subfigure[]{
...
\end{center}
\end{figure}
If the line rupture option is chosen, multiple strategies are available to take into account the rupture orientation. If a strike value is provided, ruptures can be aligned according to it, if it is not  a random strike is generated (option \Verb+Line Sources (random or given strike)+, (a) and (b)). Possible alternatives are also distributing ruptures along two perpendicular directions (NS-EW, option \Verb+Cross Hair Line Sources+, (c)), and spin the ruptures over 360\degree (every 22.5 \degree, option \Verb+16 Spoked Line Sources+, (d)). 
\begin{figure}[!htbp]
\begin{center}
\subfigure[]{
...
\caption{Different options for modeling rupture orientation in area source. Along given strike (a), random strike (b), along two perpendicular directions (c), along multiple orientations (d).}
...
\item \Verb+FAULT_RUPTURE_OFFSET+: specifies the amount by which ruptures are offset on the fault (in km). It must be greater or equal to the fault surface discretization.
...
\end{itemize}
\end{itemize}
The surface of a simple fault is modeled through a regular mesh, whose spacing is given by the fault discretization parameter. Ruptures are modeled as portions of the fault surface mesh (see Figure \ref{faultSurface}). Smaller is the surface discretization step more accurate is the modeling of fault surface geometry and fault ruptures, and also the calculation of the site-to-rupture distance. Ruptures with dimensions smaller then the discretization length are treated as points. Ruptures with dimensions equal or greater then fault surface dimensions occupy the entire fault surface. The rupture hypocenter is set in the middle point of the rupture surface.\\
\begin{figure}[!htbp]
\begin{center}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMesh.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag5.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6.eps}}
...
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6AR2.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6AR4.eps}}
...
\begin{center}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6MagLengthRel.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6AR2MagLengthRel.eps}}
\subfigure[]{
\includegraphics[width=6cm]{./oqum_Figures/simpleFaultMag6AR4MagLengthRel.eps}}
